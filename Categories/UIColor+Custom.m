//
//  UIColor+Custom.h
//  ShootStudio
//
//  Created by Tom Fewster on 30/09/2011.
//  Copyright (c) 2011 Tom Fewster. All rights reserved.
//

#import "UIColor+Custom.h"

@implementation UIColor (Custom)

+ (UIColor *)redButtonAndTextColor
{
    return [UIColor colorWithRed:197/255.0 green:65/255.0 blue:51/255.0 alpha:1.0];
}

+ (UIColor *)redHighlightColor
{
    return [UIColor colorWithRed:231/255.0 green:76/255.0 blue:60/255.0 alpha:1.0];
}

+ (UIColor *)backgroundBrightColor
{
    return [UIColor colorWithRed:241/255.0 green:236/255.0 blue:223/255.0 alpha:1.0];
}

+ (UIColor *)backgroundDarkColor
{
    return [UIColor colorWithRed:225/255.0 green:216/255.0 blue:195/255.0 alpha:1.0];
}

+ (UIColor *)brownTextColor
{
    return [UIColor colorWithRed:153/255.0 green:136/255.0 blue:120/255.0 alpha:1.0];
}

+ (UIColor *)lightBrownTextColor
{
    return [UIColor colorWithRed:182/255.0 green:173/255.0 blue:151/255.0 alpha:1.0];
}

+ (UIColor *)greyTextColor
{
    return [UIColor colorWithRed:115/255.0 green:118/255.0 blue:121/255.0 alpha:1.0];
}

+ (UIColor *)darkTextColor
{
    return [UIColor colorWithRed:54/255.0 green:47/255.0 blue:45/255.0 alpha:1.0];
}

+ (UIColor *)greenTextColor
{
    return [UIColor colorWithRed:26/255.0 green:188/255.0 blue:156/255.0 alpha:1.0];
}

+ (UIColor *)cyanHighlightColor
{
    return [UIColor colorWithRed:113/255.0 green:200/255.0 blue:185/255.0 alpha:1.0];
}

+ (UIColor *)blueTextColor
{
	return [UIColor colorWithRed:0.243 green:0.306 blue:0.435 alpha:1.0];
}

+ (UIColor *)yellowBoxColor
{
    return [UIColor colorWithRed:231/255.0 green:202/255.0 blue:132/255.0 alpha:1.0];
}
@end
