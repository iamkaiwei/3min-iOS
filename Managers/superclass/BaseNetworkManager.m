...
- (BOOL)handleServerError:(NSDictionary*)json response:(NSHTTPURLResponse*)response failure:(void (^)(NSError *error))failure
{
    ...
    {
        id errors = [json objectForKey:@"error"];
        if (errors != nil) {
            if (failure)
...
- (NSMutableDictionary*)getAuthParams
{
    ...
    //access_token
    NSString *facebook_token = [[TMEUserManager sharedInstance] getFacebookToken];
...
/*
 * Only this method should have 'cache' flag
 */
                           ...
                           failure:(void (^)(NSError *error))failure
{
    ...
    //  if (useCache)
    //  {
    //    BOOL hasCache = [[BaseStorageManager sharedInstance] hasCacheForModelCache:classObject];
    //    if (hasCache)
    //    {
    //      NSMutableArray * array = [[[BaseStorageManager sharedInstance] getAllClassModelObject:classObject] mutableCopy];
                               ...
                               failure:(void (^)(NSError *error))failure
{
    ...
    //  {
    //    //Nested param for the data model
    //    NSString * modelClassname = [self getServerClassNameForClassObject:[modelObject class]];
    //    if (modelClassname && modelObject)
    //      [params setObject:[modelObject toDictionary] forKey:modelClassname];
    //
    //    method = PUT_METHOD;
                          ...
                          failure:(void (^)(NSError *error))failure
{
    ...
    //    [[BaseStorageManager sharedInstance] deleteModelObject:modelObject];
    //
    //    if (success)
    //      success(YES);
    //
...
- (AFDownloadRequestOperation *)downloadRemoteFile:(NSString *)url
